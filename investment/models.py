...
class CreateInvest(models.Model): #CreateInvest name will change! like InvestAdvice or Invest
    investor = models.ForeignKey(MyUser,
...
class ResultInvest(models.Model):
    invest = models.OneToOneField(CreateInvest,
                                  on_delete=models.CASCADE,
                                  related_name='result')
    investor = models.ForeignKey(MyUser,
                                 on_delete=models.CASCADE,
                                 related_name='investor_result')
    member = models.ManyToManyField(MyUser,
                                    blank=True,
                                    related_name='member_result')
    ...
    total_value = models.DecimalField(max_digits=14,
                                      decimal_places=7,
                                      default=0)
    ...
    def save(self, *args, **kwargs):
        self.total_value = (self.start_value
                            - decimal.Decimal(self.result_value))
        self.ratio = (((self.start_value
                        - decimal.Decimal(self.result_value))
